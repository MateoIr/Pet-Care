...
const Home = ({ setUser }) => {
  ...
  const { reporte, isLoadingt, errort } = useGetContarTurnosPorRango(fechaDesde, fechaHasta);
  ...
  useEffect(() => {
    if (reporte && Object.keys(reporte).length > 0) {
      const xAxisData = Object.keys(reporte).map(dateStr => new Date(dateStr));
      ...
      setChartData(prevData => {
        ...
        if (
          ...
          JSON.stringify(prevData.series) !== JSON.stringify([
            { data: reporteSeriesData, label: "Ocupación de guardería por día", color: "#ff7043" },
          ])
        ) {
          return {
            xAxis: xAxisData,
        ...
        <Grid item xs={12} sm={10}  className="homeContainer">
          <Grid container rowGap={3} className="listadoTurnos">
            <Grid item md={10} rowGap={3} style={{ marginTop: "2rem" }} >
              <Box className="titlePage">Cupos asignados actualmente en la guardería: </Box>
            </Grid>
            ...
            <Grid item md={10} rowGap={3} style={{ marginTop: "2rem" }} >
              <Box className="titlePage">Asignación de cupos en la guardería: </Box>
                  ...
                  {isLoadingt ? (
                    ...
                    <LineChart
                      xAxis={[{
                        ...
                        dataKey: 'date',
                        scaleType: 'point',
                        valueFormatter: (date) => date.getDate().toString()+"-"+(date.getMonth()+1).toString(),

                      }]}
                      yAxis={[{
                        label: "Cupos asignados",
                        valueFormatter: (v) => (v === null ? '' : numberFormatter(v)),
                      }]}
